...
<?php
  ...
  include_once("includes/fn/pg_connect.php");
    ...
    $db = pg_connect( "$host $port $dbname $credentials"  );
      ...
      if(!in_array($_SESSION['role_id'], array('88','99'))){
        $sql_plus = " WHERE  branch_id='".$_SESSION['branch_id']."' ";
        $sortx = "  branch_id asc, ";
      }
      ...
      $sql_sel_group  = "SELECT group_id, group_name,value_map_volte_censor,status,branch_id
	                        FROM page_data_manage_group ".$sql_plus." ORDER BY ".$sortx." group_id ASC,sort ASC;";
        ...
        <div class="py-4 w-full">
          ...
          <tbody>
            ...
            <td class="text-right border border-slate-600">Vulue Parameter Map volte_censor : </td>
              <td class="border border-slate-600">
                <input type="text" id="vulue_map_volte_censer" name="vulue_map_volte_censer" class="w-full p-2 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
              </td>
            </tr>
            <tr>
        ...
        <table class="table-auto w-4/4 m-auto border border-collapse  border-slate-500 text-center">
          <thead>
            <tr>
              ...
              <th class="border border-slate-600 py-2 px-4">Group name</th>
              <th class="border border-slate-600 py-2 px-4">Vulue Parameter <br>Map volte_censor</th>
          ...
          <tbody>
            ...
            ?>
            <tr id="tr_list_<?php echo $rs_sel_group[$i]->group_id;?>">
              ...
              <td class="border border-slate-600 py-2 px-4"><?php echo $rs_sel_group[$i]->group_name;?></td>
              <td class="border border-slate-600 py-2 px-4"><?php echo $rs_sel_group[$i]->value_map_volte_censor;?></td>
              <td class="border border-slate-600 py-2 px-4">
                <label class="inline-flex items-center cursor-pointer">
                     <input type="checkbox" value=""  id="status_group_<?php echo $rs_sel_group[$i]->group_id;?>" class="sr-only peer" <?php if($rs_sel_group[$i]->status=='1'){echo "checked";} ?> onclick="sent_sw_change('<?php echo $rs_sel_group[$i]->group_id;?>')" /><div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                </label>
              </td>
              <td class="border border-slate-600">
                <a href="manage_data_group_edit.php?group_id=<?php echo $rs_sel_group[$i]->group_id;?>">  
              ...
              <td class="border border-slate-600">
                <button
                  ...
                  type="button"
                  onclick="confirm_del_group('<?php echo $rs_sel_group[$i]->group_id;?>');"
                >❌</button></td>
            </tr>
            <?php
               }
  ...
  <script>
    function confirm_del_group(elm_index){
      if(confirm("ต้องการลบ Group นี้?") == true){
    ...
    function add_group(){
      ...
      var dataString = 'data_group_manage_type=3'+'&group_name='+group_name+'&vulue_map_volte_censer='+ vulue_map_volte_censer+"&branch_id="+branch; //ค่าที่จะส่งไปพร้อมตัวแปร
          $.ajax ({
              type: "POST", //METHOD "GET","POST"
              url: "ajax_manage_data_group_process.php", //File ที่ส่งค่าไปหา
              data: dataString,
