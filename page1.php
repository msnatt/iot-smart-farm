...
<head>
    ...
    <title>Configuration parameters page</title>
...
<?php
  ...
  include_once("includes/fn/pg_connect.php");
    ...
    $db = pg_connect( "$host $port $dbname $credentials"  );
      ...
      if(!in_array($_SESSION['role_id'], array('88','99'))){
        $sql_plus = " AND branch_id='".$_SESSION['branch_id']."' ";
        $isAdminRole = FALSE;
      }
      $arrBranch = array();
      ...
      $sql_monitor ="SELECT monitor_id, group_id, device_id, type_id, is_min, min_value, is_max, max_value, is_input_digi, is_digital_zero, is_line, input_line, is_email, input_email, is_sms, input_sms, createtime, updatetime, status, list_time_of_work,is_condition, config_value_1, config_value_2, config_value_3, config_value_4,datax_id,datax_value,monitor_name,image_banner,branch_id
                FROM page_data_manage_monitor  WHERE status='1' ".$sql_plus." ORDER BY branch_id ASC,monitor_id DESC,sort ASC;";
                // echo $sql_monitor;exit;
      $rs_monitor = select($sql_monitor,$db);
      
      $sql_sel_group  = "SELECT group_id, group_name,value_map_volte_censor,branch_id
	                        FROM page_data_manage_group WHERE status='1' ".$sql_plus." ORDER BY group_id ASC,sort ASC;";
      $rs_sel_group   = select($sql_sel_group,$db);
      $sql_sel_device  = "SELECT device_id, divice_name,branch_id
	                        FROM page_data_manage_device WHERE status='1' ".$sql_plus." ORDER BY device_id ASC,sort ASC;";
      ...
      $sql_sel_datax  = "SELECT datax_id, datax_name, create_time, update_time, sort, status ,branch_id
                            FROM page_data_manage_datax WHERE status='1' ".$sql_plus." ORDER BY datax_id ASC,sort ASC;";
    ...
    <section class="bg-card text-card-foreground py-16">
        <div class="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-lg shadow-md p-4" id="monitor_group_elm_<?php echo $rs_monitor[$i]->monitor_id;?>">
                  <?php
                    if($rs_monitor[$i]->image_banner!=''){
                      echo '<img src="'.$rs_monitor[$i]->image_banner.'" alt="Item 1" id="img_device_<?php echo $rs_monitor[$i]->monitor_id;?>" class="w-full h-40 object-cover rounded-lg mb-4" />';
                    }else{
                      ?>
                ...
                <?php 
                  ...
                  <div class='w-4/4'>
                    <p class="text-sm text-muted-foreground mb-4 float-left">
                      Monitor ID : <?php echo $rs_monitor[$i]->monitor_id;?>
                    </p>
                    <p class="text-sm text-muted-foreground mb-4 float-right">
                      <?php
                       if($isAdminRole){
                          echo "<b>[".$arrBranch[$rs_monitor[$i]->branch_id]."]</b>";
                        }
                      ?>
                      [<span id="d_group_<?php echo $rs_monitor[$i]->monitor_id;?>"><?php
                        if(count($rs_sel_group)>0){
                          for($g=0;$g<count($rs_sel_group);$g++){
                              if($rs_sel_group[$g]->group_id==$rs_monitor[$i]->group_id){echo $rs_sel_group[$g]->group_name.'['.$rs_sel_group[$g]->value_map_volte_censor.']';}
                          }
                        }
                      ?></span>/<span id="d_device_<?php echo $rs_monitor[$i]->monitor_id;?>"><?php
                      if(count($rs_sel_device)>0){
                        for($d=0;$d<count($rs_sel_device);$d++){
                            if($rs_sel_device[$d]->device_id==$rs_monitor[$i]->device_id){echo $rs_sel_device[$d]->divice_name;}
                        }
                      }
                    ?></span>/<span id="d_type_<?php echo $rs_monitor[$i]->monitor_id;?>"><?php
                    if(count($rs_sel_type)>0){
                      for($t=0;$t<count($rs_sel_type);$t++){
                        if($rs_sel_type[$t]->type_id==$rs_monitor[$i]->type_id){echo $rs_sel_type[$t]->type_name;}
                      }
                    }
                  ?></span>/<span id="d_data_<?php echo $rs_monitor[$i]->monitor_id;?>"><?php
                  if(count($rs_sel_datax)>0){
                    for($dx=0;$dx<count($rs_sel_datax);$dx++){
                        if($rs_sel_datax[$dx]->datax_id==$rs_monitor[$i]->datax_id){echo $rs_sel_datax[$dx]->datax_name;}
                    }
                  }
                ?></span>]
                    </p>
                    ...
                    <?php
                      if(count($rs_sel_group)>0){
                        for($g=0;$g<count($rs_sel_group);$g++){
                          $str_plus = "";
                          if($isAdminRole){
                            $str_plus = "[".$arrBranch[$rs_sel_group[$g]->branch_id]."]";
                          }
                          if($rs_sel_group[$g]->group_id==$rs_monitor[$i]->group_id){$selected = "selected";}else{$selected = "";}
                          echo '<option value="'.$rs_sel_group[$g]->group_id.'" '.$selected.'>'.$str_plus.$rs_sel_group[$g]->group_name.'['.$rs_sel_group[$g]->value_map_volte_censor.']</option>';
                        }
                      }
                    ?>
                    ...
                    <?php
                      if(count($rs_sel_device)>0){
                        for($d=0;$d<count($rs_sel_device);$d++){
                          $str_plus = "";
                          if($isAdminRole){
                            $str_plus = "[".$arrBranch[$rs_sel_device[$d]->branch_id]."]";
                          }
                          if($rs_sel_device[$d]->device_id==$rs_monitor[$i]->device_id){$selected = "selected";}else{$selected = "";}
                          echo '<option value="'.$rs_sel_device[$d]->device_id.'" '.$selected.'>'.$str_plus.$rs_sel_device[$d]->divice_name.'</option>';
                        }
                      }
                    ?>
                    <!-- <option selected>Choose a country</option>
                    ...
                    <?php
                      if(count($rs_sel_datax)>0){
                        for($dx=0;$dx<count($rs_sel_datax);$dx++){
                          $str_plus = "";
                          if($isAdminRole){
                            $str_plus = "[".$arrBranch[$rs_sel_datax[$dx]->branch_id]."]";
                          }
                          if($rs_sel_datax[$dx]->datax_id==$rs_monitor[$i]->datax_id){$selected = "selected";}else{$selected = "";}
                          echo '<option value="'.$rs_sel_datax[$dx]->datax_id.'" '.$selected.'>'.$str_plus.$rs_sel_datax[$dx]->datax_name.'</option>';
                        }
                      }
                    ?>
                </select>
                ...
                <h2 class="text-lg font-bold mb-2">
                  5. Banner : 
                  <a href="page1_change_banner.php?monitor_id=<?php echo $rs_monitor[$i]->monitor_id;?>">
                    <button class="align-middle bg-primary text-primary-foreground px-1 py-1 rounded-lg w-3/3 self-auto">Change Banner</button>
                  </a>
                </h2>     
              </div>
                <hr>
  ...
  <script>
    ...
    function sel_group_change(elm_index){ 
      ...
      $("#d_group_"+elm_index).html($("#sel_group_"+elm_index).find(":selected").html());
    ...
    function sel_device_change(elm_index){ 
      ...
      $("#d_device_"+elm_index).html($("#sel_device_"+elm_index).find(":selected").html());
    ...
    function sel_type_change(elm_index){ 
      ...
      $("#d_type_"+elm_index).html($("#sel_type_"+elm_index).find(":selected").html());
    ...
    function sel_datax_change(elm_index){ 
      ...
      $("#d_data_"+elm_index).html($("#sel_datax_"+elm_index).find(":selected").html());
