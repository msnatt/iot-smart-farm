...
<?php
  ...
  include_once("includes/fn/pg_connect.php");
    ...
    $db = pg_connect( "$host $port $dbname $credentials"  );
      ...
      $sql_sel_group  = "SELECT device_id, divice_name,status,branch_id
	                        FROM page_data_manage_device ".$sql_plus." ORDER BY ".$sortx." device_id ASC,sort ASC;";;
    ...
    <section class="bg-card text-card-foreground py-16">
      <div class="flex flex-col">
        <div class="py-4 w-full">
          ...
          <tbody>
            <tr>
              <td class="text-right border border-slate-600 w-1/3">Device Name : </td>
        ...
        <table class="table-auto w-4/4 m-auto border border-collapse  border-slate-500 text-center">
          <thead>
            <tr>
              ...
              <th class="border border-slate-600 py-2 px-4">Device name</th>
          ...
          <tbody>
            ...
            ?>
            <tr id="tr_list_<?php echo $rs_sel_group[$i]->device_id;?>">
              ...
              <td class="border border-slate-600 py-2 px-4"><?php echo $rs_sel_group[$i]->divice_name;?></td>
              <td class="border border-slate-600 py-2 px-4">
                <label class="inline-flex items-center cursor-pointer">
                     <input type="checkbox" value=""  id="status_group_<?php echo $rs_sel_group[$i]->device_id;?>" class="sr-only peer" <?php if($rs_sel_group[$i]->status=='1'){echo "checked";} ?> onclick="sent_sw_change('<?php echo $rs_sel_group[$i]->device_id;?>')" /><div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                </label>
              </td>
              <td class="border border-slate-600">
                <a href="manage_data_device_edit.php?device_id=<?php echo $rs_sel_group[$i]->device_id;?>">  
              ...
              <td class="border border-slate-600">
                <button
                  ...
                  type="button"
                  onclick="confirm_del_group('<?php echo $rs_sel_group[$i]->device_id;?>');"
                >❌</button></td>
            </tr>
            <?php
               }
      ...
      if(confirm("ต้องการลบ device นี้?") == true){
        var dataString = 'data_group_manage_type=1'+'&group_id='+elm_index; //ค่าที่จะส่งไปพร้อมตัวแปร
          $.ajax ({
              ...
              url: "ajax_manage_data_device_process.php", //File ที่ส่งค่าไปหา
              data: dataString,
              //cache: false,
