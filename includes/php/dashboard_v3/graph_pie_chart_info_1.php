...
<div class="w-11/12 
            min-[320px]:w-[60%] min-[320px]:h-[auto] 
            xl:w-[23.3%] 
            min-h-[309px] 
            2xl:h-[309px] 
            md:w-[63%] 
...
<!-- <div class="w-full xl:w-1/5 min-h-[309px] bg-white rounded-lg shadow dark:bg-gray-800 p-4 md:p-6 float-left"> -->
<h2 class="semi_pie "><?php echo $item_name;?><?php echo $branch_name;?></h2>
  ...
  <!-- Line Chart -->
  <div style="margin:0 auto;" class="py-6" id="pie-chart_<?php echo $dashboard_id;?>" ></div>
...
<?php
  ...
  // print_r($monitor_id);
  $temp_value = array();
  $label_name_show = array();
  $label_color_code_show = array();
  if(count($monitor_id)>0){
    for($j=0;$j<count($monitor_id);$j++){
      ...
      $label_color_code_show[] = '"'.$label_color_code[$monitor_id[$j]][0].'"';
    }
  }
  $series = join(",",$temp_value);
  $colors = join(",",$label_color_code_show);
  $labels = join(",",$label_name_show);
  // print_r($temp_value);
  // exit;
...
const getChartOptions_<?php echo $dashboard_id;?> = () => {
  return {
...
if (document.getElementById("pie-chart_<?php echo $dashboard_id;?>") && typeof ApexCharts !== 'undefined') {
  const chart = new ApexCharts(document.getElementById("pie-chart_<?php echo $dashboard_id;?>"), getChartOptions_<?php echo $dashboard_id;?>());
  chart.render();
}
</script>