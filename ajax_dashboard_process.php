<?php
 ...
 if(!$db) {
    ...
    if($page1_sql_type == '1'){
      ...
      $sql = "SELECT  a.dashboard_item_id, a.status, b.monitor_id, b.group_id, b.device_id, b.type_id,b.datax_id
                      FROM dashboard_item_sub_data a LEFT JOIN page_data_manage_monitor b
                        ON a.monitor_id = b.monitor_id
                      WHERE 	a.status='1' AND a.dashboard_item_id ='{$dashboard_id}';";
      $rs   = select($sql,$db);
      if(count($rs)!='1'){
        $msg = "dashboard_item_id is wrong";
        $respose = array('ret'=>'109','msg' => $msg);
        echo json_encode($respose);
        exit;
      }
      $monitor_id = $rs[0]->monitor_id;
      $data_location = $rs[0]->group_id;
      $data_name = $rs[0]->device_id;
      $datax_id = $rs[0]->datax_id;
